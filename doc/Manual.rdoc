...
expiry date for each associated payment record will be extended by the subscription period in the
...
If a purchase is cancelled while the paid balance is zero, all associated payment records and
unshipped product orders will be deleted.  If a purchase is cancelled or reversed once the paid
balance is nonzero, the subscription expiry date for each associated payment record will be reduced
...
payment records, subscriptions and product orders.
