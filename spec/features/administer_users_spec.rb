...
RSpec.feature 'Administer admin users', type: :feature do
  ...
  context 'when logged in' do
    ...
    context 'when showing index' do
      ...
      [ :name, :email, :customers, :current_sign_in_at, :sign_in_count, :created_at, :updated_at ].each do |field|
    ...
    context 'when viewing record' do
      ...
      [ :name, :email, :preferred_currency, :customers, :unconfirmed_email, :confirmation_sent_at, :confirmed_at,
        :reset_password_sent_at, :remember_created_at, :sign_in_count, :current_sign_in_at, :last_sign_in_at,
        :current_sign_in_ip, :last_sign_in_ip, :failed_attempts, :locked_at, :created_at, :updated_at ].each do |field|
        scenario { expect(page).to have_css :th, text: field.to_s.titlecase }
      end
    end
